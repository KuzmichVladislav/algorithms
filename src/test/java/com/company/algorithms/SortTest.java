...
class SortTest {
   ...
   void testBubbleSort() {
      ...
      System.out.println("Time taken for sorting 100000 elements using Bubble Sort: " + duration + " nanoseconds");
   ...
   void testInsertionSort() {
      ...
      long endTime = System.nanoTime();
      long duration = (endTime - startTime);
      System.out.println("Time taken for sorting 100000 elements using Insertion Sort: " + duration + " nanoseconds");
      for (int i = 0; i < arr.length - 1; i++) {
         Assertions.assertTrue(arr[i] <= arr[i + 1]);
      }
   }
